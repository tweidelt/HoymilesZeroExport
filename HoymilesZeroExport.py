#!/opt/HoymilesZeroExport/bin/python3
...
try:
    ...
    config.read(
        str(
            Path.joinpath(
                Path(__file__).parent.resolve(), "HoymilesZeroExport_Config.ini"
            )
        )
    )
...
try:
    ...
    logger.info(
        "Error: your Python version is too old, this script requires version 3.6 or newer. Please update your Python."
    )
    sys.exit()
...
def SetLimitOpenDTU(pInverterId, pLimit):
    ...
    logger.info(
        'OpenDTU: Inverter "%s": setting new limit from %s Watt to %s Watt',
        NAME[pInverterId],
        CastToInt(CURRENT_LIMIT[pInverterId]),
        CastToInt(pLimit),
    )
    r = requests.post(
        url, data=data, auth=HTTPBasicAuth(OPENDTU_USER, OPENDTU_PASS), headers=headers
    )
    r.raise_for_status()
...
def SetLimitAhoy(pInverterId, pLimit):
    ...
    logger.info(
        'Ahoy: Inverter "%s": setting new limit from %s Watt to %s Watt',
        NAME[pInverterId],
        CastToInt(CURRENT_LIMIT[pInverterId]),
        CastToInt(pLimit),
    )
...
def WaitForAckOpenDTU(pInverterId, pTimeoutInS):
    ...
    while time.time() < timeout_start + timeout:
        ...
        ParsedData = requests.get(
            url, auth=HTTPBasicAuth(OPENDTU_USER, OPENDTU_PASS), timeout=10
        ).json()
    ...
    try:
        if SET_LIMIT_RETRY != -1:
            ...
            if (SetLimit.LastLimit == pLimit) and SetLimit.LastLimitAck:
                logger.info("Inverter limit already at %s Watt", CastToInt(pLimit))
                return
            ...
            if SetLimit.SameLimitCnt >= SET_LIMIT_RETRY:
                logger.info(
                    "Retry Counter exceeded: Inverterlimit already at %s Watt",
                    CastToInt(pLimit),
                )
...
def GetHoymilesInfoOpenDTU(pInverterId):
    ...
    TEMPERATURE[pInverterId] = (
        str(
            round(
                float(
                    (
                        ParsedData["inverters"][pInverterId]["INV"]["0"]["Temperature"][
                            "v"
                        ]
                    )
                ),
    ...
    logger.info(
        'OpenDTU: Inverter "%s" / serial number "%s" / temperature %s',
        NAME[pInverterId],
        SERIAL_NUMBER[pInverterId],
        TEMPERATURE[pInverterId],
    )
...
def GetHoymilesInfoAhoy(pInverterId):
    ...
    logger.info(
        'Ahoy: Inverter "%s" / serial number "%s" / temperature %s',
        NAME[pInverterId],
        SERIAL_NUMBER[pInverterId],
        TEMPERATURE[pInverterId],
    )
    ...
    try:
        for i in range(INVERTER_COUNT):
            try:
                ...
                logger.error(
                    "Exception at GetHoymilesInfo, Inverter %s not reachable", i
                )
                if hasattr(e, "message"):
                    logger.error(e.message)
                else:
...
def GetHoymilesPanelMinVoltageOpenDTU(pInverterId):
    ...
    for i in range(len(ParsedData["inverters"][pInverterId]["DC"])):
        if i not in ExcludedPanels:
            PanelVDC.append(
                float(
                    ParsedData["inverters"][pInverterId]["DC"][str(i)]["Voltage"]["v"]
                )
            )
    minVdc = float("inf")
    for i in range(len(PanelVDC)):
...
def GetHoymilesPanelMinVoltage(pInverterId):
    try:
        try:
            ...
            logger.error(
                "Exception at GetHoymilesPanelMinVoltage, Inverter %s not reachable",
                pInverterId,
            )
    except:
    ...
    try:
        ...
        if SET_LIMIT_RETRY != -1:
            ...
            if SetLimit.LastPowerStatus[pInverterId] == pActive:
                SetLimit.SamePowerStatusCnt[pInverterId] = (
...
def GetHoymilesTemperatureOpenDTU(pInverterId):
    ...
    logger.info(
        'OpenDTU: Inverter "%s" temperature: %s',
        NAME[pInverterId],
        TEMPERATURE[pInverterId],
    )
...
def GetHoymilesActualPowerOpenDTU(pInverterId):
    ...
    ActualPower = CastToInt(
        ParsedData["inverters"][pInverterId]["AC"]["0"]["Power"]["v"]
    )
    logger.info(
        'OpenDTU: Inverter "%s" power producing: %s %s',
        NAME[pInverterId],
        ActualPower,
        " Watt",
    )
...
def GetHoymilesActualPowerAhoy(pInverterId):
    ...
    logger.info(
        'Ahoy: Inverter "%s" power producing: %s %s',
        NAME[pInverterId],
        ActualPower,
        " Watt",
    )
...
def GetPowermeterWattsTasmota_Intermediate():
    ...
    ParsedData = requests.get(url, timeout=10).json()
    Watts = CastToInt(
        ParsedData[TASMOTA_JSON_STATUS_INTERMEDIATE][
            TASMOTA_JSON_PAYLOAD_MQTT_PREFIX_INTERMEDIATE
...
def GetPowermeterWattsShelly1PM_Intermediate():
    ...
    ParsedData = requests.get(
        url,
        headers=headers,
        auth=(SHELLY_USER_INTERMEDIATE, SHELLY_PASS_INTERMEDIATE),
        timeout=10,
    ).json()
...
def GetPowermeterWattsShellyPlus1PM_Intermediate():
    ...
    ParsedData = requests.get(
        url,
        headers=headers,
        auth=HTTPDigestAuth(SHELLY_USER_INTERMEDIATE, SHELLY_PASS_INTERMEDIATE),
        timeout=10,
    ).json()
...
def GetPowermeterWattsHomeAssistant_Intermediate():
    ...
    headers = {
        "Authorization": "Bearer " + HA_ACCESSTOKEN_INTERMEDIATE,
...
def GetPowermeterWattsVZLogger_Intermediate():
    url = (
        f"http://{VZL_IP_INTERMEDIATE}:{VZL_PORT_INTERMEDIATE}/{VZL_UUID_INTERMEDIATE}"
    )
    ParsedData = requests.get(url, timeout=10).json()
...
def GetPowermeterWattsTasmota():
    ...
    if not TASMOTA_JSON_POWER_CALCULATE:
        Watts = CastToInt(
            ParsedData[TASMOTA_JSON_STATUS][TASMOTA_JSON_PAYLOAD_MQTT_PREFIX][
                TASMOTA_JSON_POWER_MQTT_LABEL
            ]
        )
    else:
        input = ParsedData[TASMOTA_JSON_STATUS][TASMOTA_JSON_PAYLOAD_MQTT_PREFIX][
            TASMOTA_JSON_POWER_INPUT_MQTT_LABEL
        ]
        ouput = ParsedData[TASMOTA_JSON_STATUS][TASMOTA_JSON_PAYLOAD_MQTT_PREFIX][
...
def GetPowermeterWattsShellyEM():
    ...
    headers = {"content-type": "application/json"}
    ParsedData = requests.get(
        url, headers=headers, auth=(SHELLY_USER, SHELLY_PASS), timeout=10
    ).json()
    Watts = sum(CastToInt(emeter["power"]) for emeter in ParsedData["emeters"])
...
def GetPowermeterWattsShelly3EMPro():
    ...
    headers = {"content-type": "application/json"}
    ParsedData = requests.get(
        url, headers=headers, auth=HTTPDigestAuth(SHELLY_USER, SHELLY_PASS), timeout=10
    ).json()
    Watts = CastToInt(ParsedData["total_act_power"])
...
def GetPowermeterWattsEmlog():
    ...
    ParsedData = requests.get(url, timeout=10).json()
    Watts = CastToInt(
        CastToInt(ParsedData["Leistung170"]) - CastToInt(ParsedData["Leistung270"])
    )
...
def GetPowermeterWattsHomeAssistant():
    if not HA_POWER_CALCULATE:
        ...
        headers = {
            "Authorization": "Bearer " + HA_ACCESSTOKEN,
            "content-type": "application/json",
        }
        ParsedData = requests.get(url, headers=headers, timeout=10).json()
        Watts = CastToInt(ParsedData["state"])
...
def GetPowermeterWattsModbusTCP():
    regCount = 2 if MODBUS_TCP_REGISTER_TYPE == "int32" else 1
    regs = modbusClient.read_holding_registers(MODBUS_TCP_REGISTER, regCount)
    Watts = 0
    if regs is not None:
        if (MODBUS_TCP_REGISTER_TYPE == "int32"):
            bytes = struct.pack('>HH', regs[1], regs[0])
            Watts = int.from_bytes(bytes, byteorder='big', signed=True) * MODBUS_TCP_REGISTER_SCALE
        else:
            Watts = CastToInt(regs[0]) * MODBUS_TCP_REGISTER_SCALE
...
def GetPowermeterWatts():
    try:
        if USE_SHELLY_EM:
            ...
            return GetPowermeterWattsModbusTCP()
        else:
...
logger.info(
    "read config file: "
    + str(
        Path.joinpath(Path(__file__).parent.resolve(), "HoymilesZeroExport_Config.ini")
    )
)
...
USE_MODBUS_TCP = config.getboolean("SELECT_POWERMETER", "USE_MODBUS_TCP")
...
TASMOTA_JSON_PAYLOAD_MQTT_PREFIX = config.get(
    "TASMOTA", "TASMOTA_JSON_PAYLOAD_MQTT_PREFIX"
)
TASMOTA_JSON_POWER_MQTT_LABEL = config.get("TASMOTA", "TASMOTA_JSON_POWER_MQTT_LABEL")
TASMOTA_JSON_POWER_CALCULATE = config.getboolean(
    "TASMOTA", "TASMOTA_JSON_POWER_CALCULATE"
)
TASMOTA_JSON_POWER_INPUT_MQTT_LABEL = config.get(
    "TASMOTA", "TASMOTA_JSON_POWER_INPUT_MQTT_LABEL"
)
TASMOTA_JSON_POWER_OUTPUT_MQTT_LABEL = config.get(
    "TASMOTA", "TASMOTA_JSON_POWER_OUTPUT_MQTT_LABEL"
)
...
MODBUS_TCP_IP = config.get("MODBUS_TCP", "MODBUS_TCP_IP")
MODBUS_TCP_UNIT_ID = CastToInt(config.get("MODBUS_TCP", "MODBUS_TCP_UNIT_ID"))
MODBUS_TCP_REGISTER = CastToInt(config.get("MODBUS_TCP", "MODBUS_TCP_REGISTER"))
MODBUS_TCP_REGISTER_TYPE = config.get("MODBUS_TCP", "MODBUS_TCP_REGISTER_TYPE")
MODBUS_TCP_REGISTER_SCALE = float(config.get("MODBUS_TCP", "MODBUS_TCP_REGISTER_SCALE"))
USE_TASMOTA_INTERMEDIATE = config.getboolean(
    "SELECT_INTERMEDIATE_METER", "USE_TASMOTA_INTERMEDIATE"
)
USE_SHELLY_EM_INTERMEDIATE = config.getboolean(
    "SELECT_INTERMEDIATE_METER", "USE_SHELLY_EM_INTERMEDIATE"
)
USE_SHELLY_3EM_INTERMEDIATE = config.getboolean(
    "SELECT_INTERMEDIATE_METER", "USE_SHELLY_3EM_INTERMEDIATE"
)
USE_SHELLY_3EM_PRO_INTERMEDIATE = config.getboolean(
    "SELECT_INTERMEDIATE_METER", "USE_SHELLY_3EM_PRO_INTERMEDIATE"
)
USE_SHELLY_1PM_INTERMEDIATE = config.getboolean(
    "SELECT_INTERMEDIATE_METER", "USE_SHELLY_1PM_INTERMEDIATE"
)
USE_SHELLY_PLUS_1PM_INTERMEDIATE = config.getboolean(
    "SELECT_INTERMEDIATE_METER", "USE_SHELLY_PLUS_1PM_INTERMEDIATE"
)
USE_SHRDZM_INTERMEDIATE = config.getboolean(
    "SELECT_INTERMEDIATE_METER", "USE_SHRDZM_INTERMEDIATE"
)
USE_EMLOG_INTERMEDIATE = config.getboolean(
    "SELECT_INTERMEDIATE_METER", "USE_EMLOG_INTERMEDIATE"
)
USE_IOBROKER_INTERMEDIATE = config.getboolean(
    "SELECT_INTERMEDIATE_METER", "USE_IOBROKER_INTERMEDIATE"
)
USE_HOMEASSISTANT_INTERMEDIATE = config.getboolean(
    "SELECT_INTERMEDIATE_METER", "USE_HOMEASSISTANT_INTERMEDIATE"
)
USE_VZLOGGER_INTERMEDIATE = config.getboolean(
    "SELECT_INTERMEDIATE_METER", "USE_VZLOGGER_INTERMEDIATE"
)
TASMOTA_IP_INTERMEDIATE = config.get("INTERMEDIATE_TASMOTA", "TASMOTA_IP_INTERMEDIATE")
TASMOTA_JSON_STATUS_INTERMEDIATE = config.get(
    "INTERMEDIATE_TASMOTA", "TASMOTA_JSON_STATUS_INTERMEDIATE"
)
TASMOTA_JSON_PAYLOAD_MQTT_PREFIX_INTERMEDIATE = config.get(
    "INTERMEDIATE_TASMOTA", "TASMOTA_JSON_PAYLOAD_MQTT_PREFIX_INTERMEDIATE"
)
TASMOTA_JSON_POWER_MQTT_LABEL_INTERMEDIATE = config.get(
    "INTERMEDIATE_TASMOTA", "TASMOTA_JSON_POWER_MQTT_LABEL_INTERMEDIATE"
)
...
EMLOG_METERINDEX_INTERMEDIATE = config.get(
    "INTERMEDIATE_EMLOG", "EMLOG_METERINDEX_INTERMEDIATE"
)
IOBROKER_IP_INTERMEDIATE = config.get(
    "INTERMEDIATE_IOBROKER", "IOBROKER_IP_INTERMEDIATE"
)
IOBROKER_PORT_INTERMEDIATE = config.get(
    "INTERMEDIATE_IOBROKER", "IOBROKER_PORT_INTERMEDIATE"
)
IOBROKER_CURRENT_POWER_ALIAS_INTERMEDIATE = config.get(
    "INTERMEDIATE_IOBROKER", "IOBROKER_CURRENT_POWER_ALIAS_INTERMEDIATE"
)
...
HA_ACCESSTOKEN_INTERMEDIATE = config.get(
    "INTERMEDIATE_HOMEASSISTANT", "HA_ACCESSTOKEN_INTERMEDIATE"
)
HA_CURRENT_POWER_ENTITY_INTERMEDIATE = config.get(
    "INTERMEDIATE_HOMEASSISTANT", "HA_CURRENT_POWER_ENTITY_INTERMEDIATE"
)
...
SET_LIMIT_DELAY_IN_SECONDS_MULTIPLE_INVERTER = config.getint(
    "COMMON", "SET_LIMIT_DELAY_IN_SECONDS_MULTIPLE_INVERTER"
)
SET_POWER_STATUS_DELAY_IN_SECONDS = config.getint(
    "COMMON", "SET_POWER_STATUS_DELAY_IN_SECONDS"
)
POLL_INTERVAL_IN_SECONDS = config.getint("COMMON", "POLL_INTERVAL_IN_SECONDS")
ON_GRID_USAGE_JUMP_TO_LIMIT_PERCENT = config.getint(
    "COMMON", "ON_GRID_USAGE_JUMP_TO_LIMIT_PERCENT"
)
MAX_DIFFERENCE_BETWEEN_LIMIT_AND_OUTPUTPOWER = config.getint(
    "COMMON", "MAX_DIFFERENCE_BETWEEN_LIMIT_AND_OUTPUTPOWER"
)
...
if POWERMETER_MAX_POINT < (POWERMETER_TARGET_POINT + POWERMETER_TOLERANCE):
    ...
    logger.info(
        "Warning: POWERMETER_MAX_POINT < POWERMETER_TARGET_POINT + POWERMETER_TOLERANCE. Setting POWERMETER_MAX_POINT to "
        + str(POWERMETER_MAX_POINT)
    )
SERIAL_NUMBER = []
NAME = []
TEMPERATURE = []
...
for i in range(INVERTER_COUNT):
    ...
    HOY_MIN_WATT.append(
        int(
            HOY_MAX_WATT[i]
            * config.getint("INVERTER_" + str(i + 1), "HOY_MIN_WATT_IN_PERCENT")
            / 100
        )
    )
    CURRENT_LIMIT.append(int(0))
    AVAILABLE.append(bool(False))
    HOY_BATTERY_MODE.append(
        config.getboolean("INVERTER_" + str(i + 1), "HOY_BATTERY_MODE")
    )
    HOY_BATTERY_THRESHOLD_OFF_LIMIT_IN_V.append(
        config.getfloat(
            "INVERTER_" + str(i + 1), "HOY_BATTERY_THRESHOLD_OFF_LIMIT_IN_V"
        )
    )
    HOY_BATTERY_THRESHOLD_REDUCE_LIMIT_IN_V.append(
        config.getfloat(
            "INVERTER_" + str(i + 1), "HOY_BATTERY_THRESHOLD_REDUCE_LIMIT_IN_V"
        )
    )
    HOY_BATTERY_THRESHOLD_NORMAL_LIMIT_IN_V.append(
        config.getfloat(
            "INVERTER_" + str(i + 1), "HOY_BATTERY_THRESHOLD_NORMAL_LIMIT_IN_V"
        )
    )
    HOY_BATTERY_NORMAL_WATT.append(
        config.getint("INVERTER_" + str(i + 1), "HOY_BATTERY_NORMAL_WATT")
    )
    if HOY_BATTERY_NORMAL_WATT[i] > HOY_MAX_WATT[i]:
        HOY_BATTERY_NORMAL_WATT[i] = HOY_MAX_WATT[i]
    HOY_BATTERY_REDUCE_WATT.append(
        config.getint("INVERTER_" + str(i + 1), "HOY_BATTERY_REDUCE_WATT")
    )
    HOY_BATTERY_THRESHOLD_ON_LIMIT_IN_V.append(
        config.getfloat("INVERTER_" + str(i + 1), "HOY_BATTERY_THRESHOLD_ON_LIMIT_IN_V")
    )
    HOY_POWER_STATUS.append(bool(True))
    HOY_COMPENSATE_WATT_FACTOR.append(
        config.getfloat("INVERTER_" + str(i + 1), "HOY_COMPENSATE_WATT_FACTOR")
    )
    HOY_BATTERY_IGNORE_PANELS.append(
        config.get("INVERTER_" + str(i + 1), "HOY_BATTERY_IGNORE_PANELS")
    )
SLOW_APPROX_LIMIT = CastToInt(
    GetMaxWattFromAllInverters()
    * config.getint("COMMON", "SLOW_APPROX_LIMIT_IN_PERCENT")
    / 100
)
...
try:
    ...
    if USE_AHOY:
        CheckAhoyVersion()
    if USE_MODBUS_TCP:
        modbusClient = ModbusClient(host=MODBUS_TCP_IP, port=502, unit_id=MODBUS_TCP_UNIT_ID, auto_open=True)
        ...
        if GetHoymilesAvailable() and GetCheckBattery():
                ...
                if powermeterWatts > POWERMETER_MAX_POINT:
                    if ON_GRID_USAGE_JUMP_TO_LIMIT_PERCENT > 0:
                        newLimitSetpoint = CastToInt(
                            GetMaxInverterWattFromAllInverters()
                            * ON_GRID_USAGE_JUMP_TO_LIMIT_PERCENT
                            / 100
                        )
                        if (newLimitSetpoint <= PreviousLimitSetpoint) and (
                            ON_GRID_USAGE_JUMP_TO_LIMIT_PERCENT != 100
                        ):
                            newLimitSetpoint = (
                                PreviousLimitSetpoint
                                + powermeterWatts
                    ...
                    SetLimit(newLimitSetpoint)
                    if (
                        CastToInt(LOOP_INTERVAL_IN_SECONDS)
                        - SET_LIMIT_DELAY_IN_SECONDS
                        - x * POLL_INTERVAL_IN_SECONDS
                        <= 0
                    ):
                        break
                    else:
            ...
            if powermeterWatts < (POWERMETER_TARGET_POINT - POWERMETER_TOLERANCE):
                if PreviousLimitSetpoint >= GetMaxWattFromAllInverters():
                    ...
                    newLimitSetpoint = (
                        hoymilesActualPower + powermeterWatts - POWERMETER_TARGET_POINT
                    )
